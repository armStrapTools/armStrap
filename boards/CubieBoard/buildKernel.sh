...
function buildKernel {
  ...
  configKernel "${BUILD_ARCH}" "${BUILD_ARCH_COMPILER}" "${BUILD_KERNEL_DIR}" "${BUILD_BOARD_KERNEL}"
  
  patchKernel "${BUILD_KERNEL_DIR}"

  editConfig "${BUILD_KERNEL_CNF}" "CONFIG_CMDLINE" "${BUILD_CONFIG_CMDLINE}"  

  menuConfig "${BUILD_ARCH}" "${BUILD_ARCH_COMPILER}" "${BUILD_KERNEL_DIR}"
  
  kernelVersion "${BUILD_ARCH}" "${BUILD_ARCH_COMPILER}" "${BUILD_KERNEL_DIR}"

  makeKernel "${BUILD_ARCH}" "${BUILD_ARCH_COMPILER}" "${BUILD_KERNEL_DIR}" "${BUILD_KERNEL_NAME} modules"

  makeKernel "${BUILD_ARCH}" "${BUILD_ARCH_COMPILER}" "${BUILD_KERNEL_DIR}" "INSTALL_HDR_PATH=${BUILD_MNT_ROOT}/usr INSTALL_MOD_PATH=${BUILD_MNT_ROOT} modules_install headers_install"
