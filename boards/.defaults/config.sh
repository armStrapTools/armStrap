...
if [ -z "${BOARD_ROOTFS}" ]; then
  BOARD_ROOTFS="${BOARD_CPU_ARCH}${BOARD_CPU_FAMILY}"
fi

if [ ! -z "${ARMSTRAP_ROOTFS_FAMILY}" ]; then
  BOARD_ROOTFS_FAMILY="${ARMSTRAP_ROOTFS_FAMILY}"
fi
...
BOARD_ROOTFS_PACKAGE="${BOARD_ROOTFS}-${BOARD_ROOTFS_FAMILY}-${BOARD_ROOTFS_VERSION}.txz"
...
BOARD_DPKG_LOCALPACKAGES="${BOARD_DPKG_LOCALPACKAGES}"
...
BOARD_FSTAB=("${BOARD_LOADER_ROOT}:/:ext4:defaults:0:1")
