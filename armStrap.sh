...
if [ ! -z "${ARMSTRAP_UBUILDER}" ]; then
  ...
  makeFex "${BUILD_SBUILDER_CONFIG}" "${BUILD_UBUILDER_FAMILLY}" "${ARMSTRAP_BIN}"
  ...
  fex2bin "${BUILD_TBUILDER_SOURCE}" "${ARMSTRAP_BIN}/${BUILD_UBUILDER_FAMILLY}/${BUILD_UBUILDER_FAMILLY}.fex" "${ARMSTRAP_BIN}/${BUILD_UBUILDER_FAMILLY}/${BUILD_UBUILDER_FAMILLY}.bin"
  printStatus "armStrap" "Compressing ${BUILD_UBUILDER_FAMILLY}-u-boot files to ${ARMSTRAP_BIN}"
  ${BUILD_ARMBIAN_COMPRESS} "${ARMSTRAP_BIN}/${BUILD_UBUILDER_FAMILLY}-u-boot.txz" -C "${ARMSTRAP_BIN}/${BUILD_UBUILDER_FAMILLY}" --one-file-system . >> ${ARMSTRAP_LOG_FILE} 2>&1
  rm -rf "${ARMSTRAP_BIN}/${BUILD_UBUILDER_FAMILLY}"
  ARMSTRAP_DONE="Yes"
fi
